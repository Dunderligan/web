...
ALTER TABLE "match" DROP CONSTRAINT "group_xor_division", ADD CONSTRAINT "group_xor_division" CHECK ((("group_id" is null or "bracket_id" is null) and ("group_id" is not null or "bracket_id" is not null)));--> statement-breakpoint
ALTER TABLE "member" DROP CONSTRAINT "not_both_legacy_sr_and_tier", ADD CONSTRAINT "not_both_legacy_sr_and_tier" CHECK (("sr" is null or "tier" is null));